...
(def kvs-mux-test
  (test-suite "test for persist/kvs-mux"
    (test-case "open close db twice"
      (prn 1)
      (def c (open-db-connection "testdb-mux-oc"))
      (close-db-connection! c)
      (prn 2)
      (def c2 (open-db-connection "testdb-mux-oc"))
    ...
    (test-case "test-trivial-testdb-put-get"
      ...
      (with-db-connection (c "testdb-mux-pg")
    ...
    (test-case "test-trivial-testdb-commits"
      ...
      (def (test-commit i)
        ...
        (try
         (match (current-db-transaction)
           ((KvsMuxTx km txid status)
            (printf "~s\n" [LEAKED-TX-PARAMETER: i km txid status])
            (exit 1)
            (push! [LEAKED-TX-PARAMETER: i km txid status] failures))
           (#f (void)))
      ...
      (with-db-connection (c "testdb-mux-commits")
